...
- **Roles:** `pt`, `admin`, `officer`, `warden`, `it_admin`, `bh_clinician`
  (reserved — BH not public), `wellness_participant` — granted only by admin
- **Multi-facility (rolling out):** RLS by facility membership; staff may switch
  facilities when enabled — confirm activation before assuming live
